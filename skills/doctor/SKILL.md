...
4. **Content** — never applied here. The script records them as `deferred` with their owner. **Give the user the command, not the name of a skill**: a skill name is something they have to translate, and `ownerSkill` already carries the exact invocation. Say what it will ask them and roughly how long it takes, then let them choose the moment. The three that exist:
   - `1.9-current-state-init` → `/specture:knowledge reconcile --component <slug>`, one component at a time, with Plan-mode approval — `check` names the components in `current-state-missing` / `current-state-partial`; `characterize` for a component that has code but no specs.
   - `2.0-brand-brief` → `/specture:discover --marca` — five questions, brand-only mode; it does **not** re-open the questionnaire. Until it is done, `ux-design` refuses to author a design system, so this one blocks Phase 3.
   - `2.0-design-system-layers` → `/specture:ux-design` — Step 4 rebuilds the semantic layer with the user. Until it is done, `design-lint contrast` has not one pair to compute and blocks.
...
| `2.0-rules-core` | mechanical | `.specture/rules.yml` predates the mandatory `framework-core` invariants (`R-FILE-001/002/003`, `R-SOLID-001`): restores the ids that are missing and raises to its floor any severity the project lowered. Never rewrites a rule's text — the wording may be sharpened or translated, only id and severity are the contract. `n/a` when there is no file yet (`1.19-rules-file` creates it from the template, core included) or when it does not parse |
| `2.0-file-org-conventions` | assisted | `conventions.md` §2 declares no **location map**, so `R-FILE-003` ("types, constants and hooks live where §2 says") points at nothing and cannot be enforced. Plan mode proposes the table from the folders actually in the repo; every slot the user does not confirm stays `sin definir`, which is the signal to ask — never a hole the agent fills in |
| `2.0-brand-brief` | content | frontend projects whose `business_requirements.md` predates v2.0.0 have no `## Identidad de Marca` (MK-nnn): nothing in the flow ever asked for brand, so the design agent supplied it from its own taste. Resolve with `/specture:discover --marca` (five questions, brand-only). Never applied automatically — the fix is a conversation, not a text transform |
| `2.0-design-system-layers` | content | `design_system.md` written before v2.0.0 has a single primitive token layer: no `color.bg.*` / `text.*` / `border.*` / `focus.*`. That family is what carries dark mode and what makes the token-only rule satisfiable; without it the contrast check has no semantic pairs and would PASS over an empty matrix. Resolve with `/specture:ux-design` (Step 4). Which grey is "surface" is a design decision, not a rename — never applied automatically |
| `2.0-design-channel` | mechanical | detects which design channel the project already works against and records it in `stack.yml` `frontend.design_channel` (+ `design_project_id` when the sync config carries one). Signals, in order: `.design-sync/config.json` · a mirror README naming a remote design project · a `*.dc.html` artboard · else `none`. The key lives in `stack.yml`, not `settings.yml`: profile filtering would make it unreachable under `lean`/`full` and fail silently |
| `2.0-design-spine` | mechanical | moves a handoff mirror`s per-component documents to `docs/03-ux-ui/components/<Nombre>.md` and rewrites every citation — without the rewrite the corpus lint emits one ERROR `broken-path` per citation on every run. Marks them `Procedencia: medido del DOM`, which is what stops a later channel pull from overwriting hand-measured anatomies the channel does not return. Only the `domain` tier is inferred; the rest of the mirror`s folders are UI categories, not tiers. The scaffolding it leaves is named for one `git rm` — a migration has no deletion primitive |
| `2.0-design-metrics-tracked` | mechanical | `.gitignore` negation for `docs/.specture-meta/design-metrics.jsonl`. Phase 03 writes its own metrics file because a design run has no `epic`, and the build reader drops every line without one while its aggregate would fold design runs into the Spec Planning Gate reading. Tracked like the build metrics (decision A7) |
