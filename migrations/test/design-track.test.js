...
const channel = byId["1.20-design-channel"];
const spine = byId["1.20-design-spine"];
...
test("1.20-design-channel detects claude-design from .design-sync/config.json, with its projectId", () => {
...
test("1.20-design-channel detects claude-design from the mirror README, and canvas from a .dc.html", () => {
...
test("1.20-design-channel is n/a without a frontend and idempotent once declared", () => {
...
test("1.20-design-spine moves each component doc, preserving its tier and marking it measured", () => {
...
test("1.20-design-spine rewrites every citation — sin esto el corpus lint emite un ERROR por cita", () => {
  const root = makeProject(MIRROR_PROJECT);
  spine.apply(contextFor(root));
...
test("1.20-design-spine names the scaffolding to remove and verifies, then is idempotent", () => {
  const root = makeProject(MIRROR_PROJECT);
...
test("1.20-design-spine is n/a on a project with no mirror", () => {
