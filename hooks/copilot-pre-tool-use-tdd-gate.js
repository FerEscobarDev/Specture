#!/usr/bin/env node
// Copilot CLI adapter for the Specture TDD Honesty Gate.
//
// The policy evaluation remains shared with the Claude hook. Only Copilot's
// top-level hook response differs from Claude's hookSpecificOutput wrapper.
...
try {
  ...
  // Copilot preToolUse command hooks fail closed on nonzero exits. Specture's
  // recovery policy is fail open; the orchestrator-side git diff remains the
